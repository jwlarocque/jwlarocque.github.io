<script lang="ts">
	...
	import navExternals from '$lib/data/navExternals.json';
	import additionalPosts from '$lib/data/additionalPosts.json';
	...
	function randEndurance() {
		return Math.random() * (MAX_ENDURANCE * 0.2) + MAX_ENDURANCE * 0.8;
	}
	...
	function boidStep(timestamp: number) {
		...
		// ctx.fillText(`frametime: ${delta}`, 10, 20);
		delta = delta / 10;
		if (delta > 5) {delta = 5} else if (delta < 1) {delta = 1;}

		let takeoff_x = Math.cos(timestamp / 1000) * 0.5;
		let takeoff_y = Math.sin(timestamp / 1000) * 0.5;
		let startle_prob = Math.sin(timestamp / STARTLE_CYCLE_MS) * RANDOM_STARTLE_PROBABILITY;
		// ctx.fillText(`startle: ${Math.round(startle_prob * 10000000)}%`, 10, 40);
		...
		for (let i = 0; i < NUM_BOIDS; i++) {
			...
			if (boid.roosting) {
				...
				boid.vx *= (1 - (tiredness * 0.1)) //* delta;
				boid.vy *= (1 - (tiredness * 0.1)) //* delta;
	...
	$effect(() => {
		...
		boids = [];
		for (let i = 0; i < NUM_BOIDS; i++) {
			// random target 100px from point in center of canvas
			let theta = Math.random() * 2 * Math.PI;
			let target = nameSvg.getPointAtLength(totalLength * i / NUM_BOIDS); // TODO: try evenly spaced as well
			boids.push({
				id: i,
				...
				target_y: target.y / 100 * targetsHeight + targetsBoundingBox.top,
				roosting: false,
				startle: 0,
			});
		}
...
<style>
	...
	#boidsCanvas::after {
		...
		height: 4em;
		top: -4em;
		background: linear-gradient(180deg, rgba(238,242,243,0) 0%, rgba(238,242,243,1) 100%);
		pointer-events: none;
		z-index: 2;
	}
