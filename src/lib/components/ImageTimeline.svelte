<script lang="ts">
	...
	let curr = $state(0);
	let desc: any = $state();
	let mainRef: any = $state();
	let descRefs: Array<any> = $state([]);
	let slideRef: any = $state();
...
<svelte:window bind:scrollY={y} onkeydown={handleWindowKeydown} />
		...
		<div id="controls">
			<p>
				<span
					...
					onclick={() => goTo(curr - 1)}
					onkeypress={() => goTo(curr - 1)}
				>
					Prev
				</span>

				<span onclick={() => goTo(curr + 1)} onkeypress={() => goTo(curr + 1)}> Next </span>
			</p>
